...
export function AddUser() {
	...
	let handleAdd = async (values) => {
		const addUser = await ADD_USER(values);

		if (addUser.message) return alert("Error");
		else alert("User Was Successfully Added");
	};
...
let CreateUser = (user, setUser) => {
	let handle = (event) => {
		let name = event.target.name;
		let value = event.target.value;

		if (name === "address.city") setUser({ ...user, address: { city: event.target.value } });
	...
	return (
		<Form>
			...
			<div className="phone">
				...
				<Field id="city" name="address.city" value={user.address.city} onChange={handle} placeholder="New Phone" />
				<div className="second-color">
